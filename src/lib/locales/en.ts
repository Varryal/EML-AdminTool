...
export default {
  ...
  notifications: {
    ...
    [NotificationCode.FILESUPDATER_LOADER_VERSION_NOT_FOUND]: `Loader version not found.`,
    [NotificationCode.FILESUPDATER_GAME_VERSION_NOT_FOUND]: `Game version not found.`,
...
<p>After that, EML AdminTool will be completely removed from your system. You can then reinstall it by following the <a href="https://emlproject.com/docs/install-eml-admintool" target="_blank">installation guide</a>.</p>`
    ...
    optionalMods: {
      title: `Optional mods`,
      subtitle: `Optional IDs are logical groups. A group is enabled or disabled as one unit.`,
      searchLabel: `Search optional mods`,
      searchPlaceholder: `Search mods...`,
      count: `{{selected}} optional groups · {{files}} MOD files`,
      identityWarning: `Changing a group ID creates a new logical identity; the old saved selection will not carry over.`,
      noMods: `No MOD files found`,
      uploadHint: `Upload MOD files to the mods/ directory first.`,
      noMatches: `No matching mods`,
      tryAnotherSearch: `Try another search.`,
      invalidGroupId: `Invalid group ID: {{id}}`,
      duplicateGroupId: `Duplicate group ID: {{id}}`,
      invalidTitle: `Invalid title for {{id}}`,
      descriptionTooLong: `Description is too long for {{id}}`,
      ...
      enabledByDefault: `Recommended for players without a saved choice`,
      required: `Required`,
      makeOptional: `Make optional`,
      chooseExisting: `Add to an existing group`,
      createNew: `Create a new group`,
      orphanTitle: `Orphan optional groups`,
      orphanDescription: `These groups have no current file. Bind a replacement file or delete the group.`,
      selectFile: `Select a MOD file`,
      bindFile: `Bind file`,
      noUnboundFile: `No unbound MOD files`,
      deleteGroup: `Delete group`,
      deleteGroupConfirm: `Delete this group? Players' saved choices for it will be discarded.`,
      hint: `Required files stay enabled for everyone. Save validates the complete group set against the current file list.`,
      saving: `Saving...`,
      conflict: `The optional-mod list changed on the server. Close and reopen this dialog before saving again.`,
      invalid: `The optional-mod groups contain invalid or conflicting values.`,
      readFailed: `The saved optional-mod metadata could not be read. No changes were published.`,
      saveFailed: `The optional-mod settings could not be saved. Please try again.`
    }
  }
}
