...
async function run() {
  ...
  readme = readme.replace(/badge\/version-([a-zA-Z0-9.\-]+)-orangered/, `badge/version-${cleanVersion.replace(/-/g, '--')}-orangered`)
  await fs.writeFile('README.md', readme)

  console.log('⚙️ Генерация release-файлов из шаблонов...')
  const templates = [{ name: 'docker-compose.prod.yml', target: 'docker/docker-compose.prod.yml' }]
  ...
  console.log('\nСледующие шаги:')
  console.log('1. Выполните npm ci, npx prisma generate --no-hints, npm run check и npm run build')
  console.log('2. Закоммитьте сгенерированные файлы в отдельную ветку и влейте их в main через проверенный PR')
  console.log(`3. Поставьте tag v${cleanVersion} на слитый commit из main`)
  console.log('4. Отправьте tag; workflow релиза проверит metadata и опубликует image')
}
